...
def data_prepration(x,label): # preparing data for training and testing as we are going to use different data
    ...
    x_features_train,x_features_test,x_labels_train,x_labels_test = train_test_split(x_features,x_labels,test_size=0.1)
...
#    print('xgboost模型的数据')
 ...
 #   clf = xgb.XGBClassifier(n_jobs = -1, n_estimators = 200)
  ...
  #  model(clf,os_data_X,data_test_X,os_data_y,data_test_y)
    ...
    knei_cf1 = KNeighborsClassifier(n_neighbors=1)
    ...
    sclf = StackingClassifier(classifiers=[knei_cf1, rf_cfl, xgb_cfl],meta_classifier = log_cfl)

    for clf, label in zip([knei_cf1, rf_cfl, xgb_cfl, sclf],
                          ['KNN','Random Forest','xgb','StackingClassifier']):
        print("now the " ,label)
        model(clf,os_data_X,data_test_X,os_data_y,data_test_y)
